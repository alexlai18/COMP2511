...
public class HoogleLoginPage implements Page {
    @Override
    public void doAction(ClientApp app, Browser page, Object input) {
        ...
        Provider Hoogle = app.getProvider(userToken.getProviderName());
        String email = userToken.getUserEmail();

        if (!Hoogle.doesUserExist(email)) {
            page.setState(new SelectPage());
            page.setPreviousState(null);
            return;
        }

        if (Hoogle.getUserLockStatus(email)) {
        ...
        if (userToken.getAccessToken() != null) {
            ...
            if (Hoogle.getLoginAttempts(email) < 2) {
                page.setPreviousState(null);
                page.setState(new SelectPage());
                Hoogle.addLoginAttempt(email);
                page.setPreviousState(null);
                page.setState(new SelectPage());
            } else {
                Hoogle.addLoginAttempt(email);
                Hoogle.lockUserUniversally(app, email);
                page.setPreviousState(new SelectPage());
                page.setState(new LockedPage());
            }
        }
